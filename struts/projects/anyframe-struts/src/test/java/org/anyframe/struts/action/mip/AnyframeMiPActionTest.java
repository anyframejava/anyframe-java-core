 ...
 * [Main Flow]
 * <ul>
 * <li>#1- Positive Case : AnyframeMiPAction을 상속받은 MiPAction의 excute가 호출 되었을
 * 때 AnyframeMiPAction의 excute method에서 HttpRequest로 넘어온 사용자 입력값을 입출력
 * DatasetList, VariableList를 분리하는 것을 검증한다.</li>
 * <li>#2- Positive Case : excute method가 호출 되었을 때 AnyframeMiPAction을
 * extends한 MiPController의 precess가 제대로 호출 되고 결과 메세지를 output VariableList에 저장하는
 * 것을 검증한다.</li>
 * <li>#3- Positive Case : MiPAction의 process method에서 Exception이 발생했을 경우
...
public class AnyframeMiPActionTest extends TestCase {
	...
	/**
	 * AnyframeMiPActopm를 상속받은 MiPAction을 맴버 변수로 선언한다.
	 */
    ...
    /**
	 * [Flow #-1] Positive Case : AnyframeMiPAction을 상속 받은
	 * MiPAction의 excute가 호출 되었을 때 AnyframeMiPAction의 excute method에서
    ...
    /**
	 * [Flow #3] Positive Case : MiPAction의 process method에서 Exception이 발생했을 경우
	 * output VariableList에 Exception 메세지가 저장되는 것을 검증한다.
	 * 
	 * @throws Exception
	 */

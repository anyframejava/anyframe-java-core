...
/**
 * MockStrutsSpringTestCaseBase is a Test Base Class for testing struts-spring integration environment
 * 
 * @author Byunghun Woo
 * @since 2008.06.18
 */
public abstract class MockStrutsSpringTestCaseBase extends MockStrutsTestCase {
	...
	private final String CONFIG_LOCATIONS = "classpath*:/webapp" + "/WEB-INF/config/spring/**/context-*.xml";
	...
	public void setUp() throws Exception {
		...
		if (WEB_APP_CTXT == null) {
			...
			context.setInitParameter(ContextLoader.CONFIG_LOCATION_PARAM, CONFIG_LOCATIONS);
			ctxLoader.initWebApplicationContext(context);
			WEB_APP_CTXT = (ConfigurableWebApplicationContext) WebApplicationContextUtils.getRequiredWebApplicationContext(context);
		} else {
			WEB_APP_CTXT.setServletContext(context);
			context.setAttribute(WebApplicationContext.ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE, WEB_APP_CTXT);
		}
	}

	protected void setTokenKey() {
		// set token key
		String tokenKey = this.getMockRequest().getSession().getAttribute(org.apache.struts.Globals.TRANSACTION_TOKEN_KEY).toString();
		addRequestParameter(org.apache.struts.taglib.html.Constants.TOKEN_KEY, tokenKey);
	}
	...
	protected void verifyMessageArr(String messageKey, String userMessage, int idx) {
		Object messageObj = (Object) this.getMockRequest().getAttribute(Globals.MESSAGE_KEY);
		String[] messages = {};

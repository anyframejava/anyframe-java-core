...
*/
package org.anyframe.struts.spring.beans;
 ...
 * @author Jonghoon Kim
 */
public class MiPVariableListPropertyValues extends MutablePropertyValues {
	 ...
	 */
	public MiPVariableListPropertyValues(VariableList variableList, boolean convertToCamelCase){
	 ...
	 * @param convertToCamelCase
	 * 			When changing to CamelCase the column name of the Dataset, then true
	 */
	public MiPVariableListPropertyValues(Dataset dataList, int rowNum, boolean convertToCamelCase){
	 ...
	 * @param convertToCamelCase
	 * 			When changing to CamelCase the Dataset's column name, then true
	 */
	public MiPVariableListPropertyValues(Dataset dataList, int rowNum, boolean isDeleted, boolean convertToCamelCase){
	...
	private static Map getVariableMap(VariableList variableList, boolean convertToCamelCase){
		...
		Iterator variableIterator =  variableMap.entrySet().iterator();
		Object key = null;
		Object value = null;
	 ...
	 * @return
	 */
	private static Object convertToCamelCase(Object key,  boolean convertToCamelCase){
		if(convertToCamelCase == false || key == null || !(key instanceof String)) return key;
	 ...
	 * @return
	 */
	private static Map<Object, Object> getVariableMap(Dataset dataList, int rowNum, boolean isDeleted, boolean convertToCamelCase){
		...
		for(int i=0;i<columnCount;i++){
			...
			if (isDeleted) 
				...
				switch(type) {
					case ColumnInfo.COLTYPE_DATE : value = dataList.getColumnAsDate(rowNum, key); break;
					case ColumnInfo.COLTYPE_DECIMAL : value = dataList.getColumnAsDouble(rowNum, key); break;
					case ColumnInfo.COLTYPE_INT : value = dataList.getColumnAsInteger(rowNum, key); break;
					default : value = dataList.getColumnAsString(rowNum, key);
				}
			}
			returnMap.put(convertToCamelCase(key, convertToCamelCase), value);
		}

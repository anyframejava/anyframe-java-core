 ...
 * It allows for the usage of direct mapping in the server side value object user inputed value
 * transmitted from the presentation layer.</p>
 * We need to implement the getValueObject method and create the VO object for mapping in the 
 * inherited Action Class for the AbstractDispatchAction in order to map the value in the value 
 * object.  The VO object implements the anyframe.common.ValueObject.
 * 
 * @author Byunghun Woo
 *
 */
public abstract class AbstractDispatchAction extends DefaultDispatchActionSupport {
	...
	*/
	protected Class[] types = { ActionMapping.class, ValueObject.class, HttpServletRequest.class,
	 ...
	 * If a specific request is requested, the process method is called with respect to the action
	     ...
	     * AbstractionDispatchAction is defined as the mapping type, the process method is called and
	     * it again calls the appropriate method using the method's name info.
	 * </p>
	 * 
	 * @param mapping
			...
			if (vo != null) {
				if (multipartResolver.isMultipart(request)) {
					try {
						...
						log.error("Cannot parse the HTTP Request. Error : " + e.getMessage());
					}
				}
	...
	/**
	 * Changes to the map type by taking the key and value of the parameter included in the request.
	 * 
	 * @param request
	 * 			The HTTP request we are processing
	 * @param keyParam
	 * 			The parameter name being the key among the parameters transmitted         
	 *          from the the Presentation Layer.
	 * @return
	 * 			The collection object including the Map
	 */
	 ...
	 * @throws Exception
	 */
	protected ValidatorErrors validate(HttpServletRequest request, ValueObject vo, String validationKey)
			throws Exception {
	 ...
	 * @throws Exception
	 */
	protected ValidatorErrors validate(HttpServletRequest request, ValueObject vo, String validationKey, int page)
			throws Exception {
		...
		if (errors.isValid() == false) {
			log.debug(errors.getErrorMessage());
		}

...
<head>
	...
	<script type="text/javascript">
		function changeTheater(fm) {
			document.searchForm.action="<c:url value='/routingDataSourceMovieFinder.do?method=list'/>";
			document.searchForm.submit();
		}
		function fncCreateMovieView() {
			document.searchForm.action="<c:url value='/routingDataSourceMovie.do?method=createView'/>";
			document.searchForm.submit();
		}	
...
<form:form modelAttribute="movie" method="post" name="searchForm">	
	<table class="scrollTable" width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
		<tbody>
			<tr class="board">
				<td class="underline" style="background-color: #CCCCCC"><spring:message code="movie.theater" /></td>
				<td onMouseOver="this.style.backgroundColor='#e4eaff';return true;" onMouseOut="this.style.backgroundColor=''; return true;" >
					<input type="radio" name="theater" onclick="changeTheater(this);" value="ABCCinema" <c:if test="${theater == 'ABCCinema' }"> checked="checked"</c:if> />ABC Cinema
				</td>
				<td onMouseOver="this.style.backgroundColor='#e4eaff';return true;" onMouseOut="this.style.backgroundColor=''; return true;" >
					<input type="radio" name="theater" onclick="changeTheater(this);" value="GoodMovieCinema" <c:if test="${theater == 'GoodMovieCinema' }"> checked="checked"</c:if> />Good Movie Cinema
				</td>
				<td onMouseOver="this.style.backgroundColor='#e4eaff';return true;" onMouseOut="this.style.backgroundColor=''; return true;" >
					<input type="radio" name="theater" onclick="changeTheater(this);" value="MovieHouseCinema" <c:if test="${theater == 'MovieHouseCinema' }"> checked="checked"</c:if>/>Movie House Cinema
				</td>
			</tr>
		</tbody>
	</table>
		...
		<tbody>
			<c:forEach var="movie" items="${movies}">
				<tr class="board" onMouseOver="this.style.backgroundColor='#e4eaff';return true;" onMouseOut="this.style.backgroundColor=''; return true;" >
					...
					<td class="underline">
						<a class="linkClass" href="${ctx}/routingDataSourceMovie.do?method=get&movieId=${movie.movieId}&theater=${theater}">${movie.title}</a>
					</td>

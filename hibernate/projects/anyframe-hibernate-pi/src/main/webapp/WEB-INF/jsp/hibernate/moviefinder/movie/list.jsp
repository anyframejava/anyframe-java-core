...
<%@ include file="/sample/common/top.jsp"%>
		<div class="location"><a href="<c:url value='/anyframe.jsp'/>">Home</a> &gt; <a href="<c:url value='/hibernateMovieFinder.do?method=list'/>">Hibernate 1.6.1-SNAPSHOT</a></div>
    </div>
    <hr />
<script type="text/javascript" src="<c:url value='/sample/javascript/CommonScript.js'/>"></script>    
<script type="text/javascript">
		function fncCreateMovieView() {
			document.location.href="<c:url value='/hibernateMovie.do?method=createView'/>";
		}	
		function fncSearchMovie() {
		   	document.searchForm.action="<c:url value='/hibernateMovieFinder.do?method=list'/>";
     		...
     		<table summary="This is list of movie">
               ...
               <tbody>
				<c:forEach var="movie" items="${movies}">
					<tr>
	                   	<td>${movie[0].name}</td>
	                    <td><a class="linkClass" href="${ctx}/hibernateMovie.do?method=get&amp;movieId=${movie[1].movieId}">${movie[1].title}</a></td>
	                    <td>${movie[1].director}</td>
	                    <td>${movie[1].actors}</td>
	                    <td class="align_center">${movie[1].ticketPrice}</td>
	                    <td class="align_center">${movie[1].releaseDate}</td>
	                </tr>
				</c:forEach>
				 </tbody>
            </table>

...
public class HibernateClobTest extends AbstractConfigurationalTransactionalTest {
	...
	public void testInsertClobDataType() {
		...
		assertNotNull(clobDataType);
		assertEquals(source.getTitle(), clobDataType.getTitle());
		assertEquals(source.getContentString(), clobDataType.getContentString());
		assertEquals(convertClobIntoString(source.getContentClob()),
	...
	public void testUpdateClobDataType() {
		...
		clobDataType.setContentString(getLargeString("Update"));
		clobDataType.setContentClob(Hibernate
		...
		clobDataType = (ClobDataType) session.get(ClobDataType.class,
				new Integer(4491));
		assertNotNull(clobDataType);
		assertEquals("Update Hibernate LOB Data Test", clobDataType.getTitle());
		assertEquals(getLargeString("Update"), clobDataType.getContentString());
		assertEquals(convertClobIntoString(Hibernate
				.createClob(getLargeString("Update"))),
				convertClobIntoString(clobDataType.getContentClob()));
	}
	...
	protected String convertClobIntoString(Clob clob) {
		...
		try {
			...
			e.printStackTrace();
			fail("fail to convert Clob Into String");
		} finally {
			if (reader != null)
				try {
					reader.reset();
	...
	private ClobDataType insertClobDataType() {
		...
		clobDataType.setContentString(getLargeString("Insert"));
		clobDataType.setContentClob(Hibernate.createClob(getLargeString()));

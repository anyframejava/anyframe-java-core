	...
	public static boolean existsDir(final String srcDir, final String fromDate, final String toDate) {
		return processIO(new IOCallback<Boolean>() {
			public Boolean doInProcessIO() throws NullPointerException, SecurityException {
				...
				if (existsDir(srcDir)) {
					String lastModifed = DateUtil.date2String(new Date(file.lastModified()));
					if (DateUtil.string2Date(lastModifed).getTime() >= DateUtil.string2Date(fromDate).getTime()
							&& DateUtil.string2Date(lastModifed).getTime() <= DateUtil.string2Date(toDate).getTime()) {
						return true;
					}
					else {
						return false;

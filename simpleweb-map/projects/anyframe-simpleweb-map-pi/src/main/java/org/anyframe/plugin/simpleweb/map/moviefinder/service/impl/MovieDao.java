...
public class MovieDao extends QueryServiceDaoSupport {
	...
	public void create(Map map) throws Exception {
		create("createMapMovie", checkNumberTypeChars(map));
	}

	public void remove(Map map) throws Exception {
		remove("removeMapMovie", map);
	}

	public void update(Map map) throws Exception {
		update("updateMapMovie", checkNumberTypeChars(map));
	}

	public Map get(Map map) throws Exception {
		return (Map) findByPk("findMapMovieByPk", map);
	}

	public Page getPagingList(Map map) throws Exception {
		int pageIndex = 1;
		if (map.get("pageIndex") != null)
			pageIndex = StringUtil.string2integer(map.get("pageIndex").toString());
		return this.findListWithPaging("findMapMovieList", map, pageIndex, pageSize, pageUnit);
	}
	...
	private Map checkNumberTypeChars(Map map) {
		...
		if (StringUtil.isEmpty(map.get("runtime").toString()))
			...
			map.put("runtime", removeChars(map.get("runtime").toString(), removeChar));
		if (StringUtil.isEmpty(map.get("ticketPrice").toString()))
			map.put("ticketPrice", "0");
		else
			map.put("ticketPrice", removeChars(map.get("ticketPrice").toString(), removeChar));

	...
	public void update(Movie movie) throws Exception {
		...
		this.getSimpleJdbcTemplate().update(
				...
				new Object[] { movie.getTitle(), movie.getDirector(),
						...
						movie.getPosterFile(), movie.getMovieId() });
		...
		return this.getSimpleJdbcTemplate().queryForObject(sql,
				new BeanPropertyRowMapper<Movie>(Movie.class) {
					public Movie mapRow(ResultSet rs, int i)
							throws SQLException {
						return new Movie(rs.getString(1), rs.getString(2),
	...
	public Page getPagingList(Movie movie, int pageIndex) throws Exception {
		...
		Page result = fetchPage(
				...
				"SELECT movie.movie_id, movie.title, movie.director, genre.genre_id, genre.name, "
						+ "movie.release_date, movie.ticket_price, movie.actors, movie.runtime, movie.now_playing, poster_file "
						+ fromSql + whereSql,
				...
				new ParameterizedRowMapper<Movie>() {
					public Movie mapRow(ResultSet rs, int i)
						...
						return new Movie(rs.getString(1), rs.getString(2), rs
								...
								.getLong(9)), rs.getString(10), rs.getString(11));
					}
				});

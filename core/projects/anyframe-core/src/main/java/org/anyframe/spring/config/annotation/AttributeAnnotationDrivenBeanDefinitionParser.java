...
/**
 * {@link BeanDefinitionParser} that parses the {@code annotation-driven}
 * element to configure a Spring MVC web application.
 * 
 * <p>
 * Responsible for:
 * <ol>
 * <li>Registering a DefaultAnnotationHandlerMapping bean for mapping HTTP
 * Servlet Requests to @Controller methods using @RequestMapping annotations.
 * <li>Registering a AnnotationMethodHandlerAdapter bean for invoking annotated @Controller
 * methods. Will configure the HandlerAdapter's
 * <code>webBindingInitializer</code> property for centrally configuring
 * {@code @Controller} {@code DataBinder} instances:
 * <ul>
 * <li>Configures the conversionService if specified, otherwise defaults to a
 * fresh {@link ConversionService} instance created by the default
 * {@link FormattingConversionServiceFactoryBean}.
 * <li>Configures the validator if specified, otherwise defaults to a fresh
 * {@link Validator} instance created by the default
 * {@link LocalValidatorFactoryBean}
 * <em>if the JSR-303 API is present on the classpath</em>.
 * <li>Configures standard
 * {@link org.springframework.http.converter.HttpMessageConverter
 * HttpMessageConverters}, including the
 * {@link Jaxb2RootElementHttpMessageConverter}
 * <em>if JAXB2 is present on the classpath</em>, and the
 * {@link MappingJacksonHttpMessageConverter}
 * <em>if Jackson is present on the classpath</em>.
 * </ul>
 * </ol>
 * 
 * @author Keith Donald
 ...
 * @since 4.5.3
 */
class AttributeAnnotationDrivenBeanDefinitionParser implements
		BeanDefinitionParser {

	private static final boolean jsr303Present = ClassUtils
			.isPresent("javax.validation.Validator",
					AttributeAnnotationDrivenBeanDefinitionParser.class
							.getClassLoader());

	private static final boolean jaxb2Present = ClassUtils
			.isPresent("javax.xml.bind.Binder",
					AttributeAnnotationDrivenBeanDefinitionParser.class
							.getClassLoader());
	...
	private static boolean romePresent = ClassUtils.isPresent(
			...
			AttributeAnnotationDrivenBeanDefinitionParser.class.getClassLoader());
	...
	public BeanDefinition parse(Element element, ParserContext parserContext) {
		...
		RootBeanDefinition annMappingDef = new RootBeanDefinition(
		...
		String annMappingName = parserContext.getReaderContext()
				.registerWithGeneratedName(annMappingDef);
		...
		annAdapterDef.getPropertyValues().add("webBindingInitializer",
				bindingDef);
		annAdapterDef.getPropertyValues().add("messageConverters",
				getMessageConverters(source));
		if (element.hasAttribute("synchronizeOnSession")) {
			annAdapterDef.getPropertyValues().add("synchronizeOnSession",
					element.getAttribute("synchronizeOnSession"));
		}
		String annAdapterName = parserContext.getReaderContext()
				.registerWithGeneratedName(annAdapterDef);
		...
		parserContext.registerComponent(new BeanComponentDefinition(
				annMappingDef, annMappingName));
		parserContext.registerComponent(new BeanComponentDefinition(
				annAdapterDef, annAdapterName));
		parserContext.registerComponent(new BeanComponentDefinition(
	...
	private ManagedList<RootBeanDefinition> getMessageConverters(Object source) {
		...
		messageConverters.add(createConverterBeanDefinition(
				XmlAwareFormHttpMessageConverter.class, source));
		if (jaxb2Present) {
			messageConverters.add(createConverterBeanDefinition(

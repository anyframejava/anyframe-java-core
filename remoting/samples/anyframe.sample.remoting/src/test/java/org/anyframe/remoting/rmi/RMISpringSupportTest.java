 ...
 */
package org.anyframe.remoting.rmi;
 ...
 *                 서비스의 기능을 테스트할 수 있는 시나리오들을 정의한 TestCase이다. 
 * <br>
 * [Characteristic]
 * <li>설정을 통해 일반 서비스를 RMI 원격 서비스로 노출하여 사용</li>
 * <li>제약 사항 존재-RMI는 통신을 위해 특정 Port를 사용하므로 방화벽을 통과하기
 * 어려움, 클라이언트와 서버에서 제공되는 서비스 모두 자바로 작성되어야 함</li>
 * <li>서버 구현: Spring에서 제공하는 org.springframework.remoting.rmi.RmiServiceExporter
 * 클래스를 이용하여 손쉽게 일반 Spring Bean으로 작성된 서비스를 RMI Service로 노출</li>
 * <li>클라이언트 구현: Spring에서 제공하는 org.springframework.remoting.rmi.RmiProxyFactoryBean 클래스 이용</li>
 * <br>
 * [Assumption]
 * <li>-서버 : ClassPathXmlApplicationContext 사용, Spring 설정 파일 이용</li>
...
public class RMISpringSupportTest extends RemotingSpringTestCase {
    ...
    protected String[] getConfigLocations() {
        ...
        // client spring configuration files
    ...
    public void onSetUpServer() throws Exception {
        ...
        context = new ClassPathXmlApplicationContext("/rmi/server/WEB-INF/context-movie-server.xml");
    }

...
<%@ include file="/sample/common/top.jsp"%>
		<div class="location"><a href="<c:url value='/anyframe.jsp'/>">Home</a> &gt; <a href="<c:url value='/mybatisMovieFinder.do?method=list'/>">Mybatis 1.1.1-SNAPSHOT</a></div>
	</div>
	<hr />
<script type="text/javascript" src="<c:url value='/sample/javascript/CommonScript.js'/>"></script>    
<script type="text/javascript">
	function fncCreateMovieView() {
		document.location.href="<c:url value='/mybatisMovie.do?method=createView'/>";
	}	
	function fncSearchMovie() {
		document.searchForm.action="<c:url value='/mybatisMovieFinder.do?method=list'/>";
					...
					<tbody>
						<c:forEach var="movie" items="${movies}">
							<tr>
								...
								<td><a class="linkClass" href="${ctx}/mybatisMovie.do?method=get&amp;movieId=${movie.movieId}">${movie.title}</a></td>

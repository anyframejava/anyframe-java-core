 ...
 * under the format key, which by default is <code>format</code>. It then
 * uses the value of this parameter to lookup the actual <code>View</code>
 ...
 * <br><br>
 * We changed org.springframework.web.servlet.view.jasperreports.JasperReportsMultiFormatView Class into org.anyframe.jasperreports.ReportsMultiFormatView Class in Anyframe.
 * <ul>
 * 	<li>In ExtendedJasperReportsMultiFormatView constructor method, we modified View Classes.</li>
 * 	<li>In renderReport method, we checked requestObject from model and set populatedReport into image servlet session.</li>
 * </ul>
 * 
 * @since 1.1.5
 * 
 ...
 * 
 */
public class ReportsMultiFormatView extends
		AbstractReportsView {
	...
	public void setFormatMappings(Properties mappingsWithClassNames) {
		...
		for (Enumeration discriminators = mappingsWithClassNames
				.propertyNames(); discriminators.hasMoreElements();) {
			String discriminator = (String) discriminators.nextElement();
			String className = mappingsWithClassNames
					.getProperty(discriminator);
			try {
				if (logger.isDebugEnabled()) {
					logger.debug("Mapped view class [" + className
							+ "] to mapping key [" + discriminator + "]");
				}
				this.formatMappings.put(discriminator, ClassUtils
						.forName(className));
	 ...
	 * determine the value of the <code>Content-Disposition</code> header for
	...
	/**
	 * Return the mappings of <code>Content-Disposition</code> header values
	...
	protected void renderReport(JasperPrint populatedReport, Map model,
		...
		AbstractReportsView view = (AbstractReportsView) BeanUtils
	...
	private void populateContentDispositionIfNecessary(
		...
		if (this.contentDispositionMappings != null) {
			...
			if (header != null) {
				if (logger.isDebugEnabled()) {
					logger.debug("Setting Content-Disposition header to: ["
							+ header + "]");
				}

			...
			remove : function(obj) {
				...
				if(obj && (!this.selected || this.get_node(obj).get(0) != this.selected.get(0) )) {
					...
					if(obj.size()) {
						...
						/* 2009.11.25 - 삭제 취소시 alert 창을 띄우지 않도록 수정 함 */
						//if(!this.callback("beforedelete",[obj.get(0), _this])) return this.error("DELETE: STOPPED BY USER");
						if(!this.callback("beforedelete",[obj.get(0), _this])) return;
						$parent = obj.parent();
					...
					if(!this.callback("beforedelete",[this.selected.get(0), _this])) return;

...
/**
 * TestCase Name : NPQueryServiceTest<br>
 * <br>
 * [Description] : NPlatform의 data 전달 객체인 DataSet과 VariableList를 이용해 Database에
 * 접근하는 NPQueryService의 TestCase다. <br>
 * [Main Flow]
 * <ul>
 * <li>#1 - Positive Case : 미리 세팅되어 있는 Database의 값을 NPQueryService의 update
 * method를 호출해 값을 수정하는 TestCase다. 수정될 값이 세팅되어 있는 DataSet를 NPQueryService
 ...
 * 동작하는지 검증한다.</li>
 * <li>#4 - Positive Case : VariantList에 조회조건을 세팅하고 NPQueryService의 find method를
...
public class NPQueryServiceTest {
	...
	public void onSetUp() {
		...
		try {
			...
			try {
				...
				try {
					statement.executeUpdate("DROP TABLE TB_NP_TEST");
				} catch (SQLException e) {
					System.out.println("Fail to DROP Table.");
				}

				statement.executeUpdate("CREATE TABLE TB_NP_TEST ( "
						+ "TEST_CHAR CHAR(10), "
	...
	/**
	 * [Flow #-1] Positive Case : 미리 세팅되어 있는 Database의 값을 NPQueryService의 update
	 * method를 호출해 값을 수정하는 TestCase다. 수정될 값이 세팅되어 있는 DataSet를 NPQueryService
	...
	public void testUpdateDataSet() {
		...
		queryMap.put(NPQueryService.QUERY_UPDATE, "updateNPQueryService");
		int resultUpdate = npQueryService.update(queryMap, makeUpdateDataSet());
		...
		assertDataSet(resultDataSet, "bbnydory00", "2012-12-01", 12345678,
				1234.5678, "Anyframe NPQueryService Test. - UPDATE");
	}
	...
	public void testProcessAllDataSet() {
		...
		Map<String, String> queryMap = new HashMap<String, String>();
		queryMap.put(NPQueryService.QUERY_UPDATE, "updateNPQueryService");
		queryMap.put(NPQueryService.QUERY_INSERT, "createNPQueryService");
		queryMap.put(NPQueryService.QUERY_DELETE, "deleteNPQueryService");
		int resultUpdate = npQueryService.update(queryMap, makeAllDataSet());
	...
	/**
	 * [Flow #-4] Positive Case : VariantList에 조회조건을 세팅하고 NPQueryService의 find
	...
	public void testFindDataSetWithVariant() {
		...
		DataSet resultDataSet = npQueryService.search("findNPQueryService",
				makeVariantList());
		Assert.assertEquals(1, resultDataSet.getRowCount());

		assertDataSet(resultDataSet, "bbnydory00", "2012-12-01", 12345678,
				1234.5678, "Anyframe NPQueryService Test.");
	}
	...
	public void testFindDataSetWithWrongQueryId() throws QueryException {
		try {
			npQueryService.search("notexistqueryid", makeVariantList());
	...
	public void testFindDataSetWithoutDynamic() throws QueryException {
		try {
			npQueryService.search("findNPQueryServiceWithoutDynamic",
					makeVariantList());
			Assert.fail("Fail to throw exception.");
		} catch (Exception e) {
			Assert.assertTrue("Fail to check exception.",
			...
			Assert
					.assertEquals(
							"Fail to compare exception message.",
							"Query Service : queryId [findNPQueryServiceWithoutDynamic] is not dynamic statements.",
							((QueryException) e).getMessage());
		}
	}
	...
	public void testFindDataSetWithWrongQuery() throws QueryException {
		try {
			npQueryService.search("findNPQueryServiceWithWrongQuery",
					makeVariantList());
			Assert.fail("Fail to throw exception.");
		} catch (Exception e) {
	...
	public void testBatchSaveAll() throws QueryException {
		try {
			...
			int resultUpdate = npQueryService.batchCreate(
					"createNPQueryService", inDs);
			resultUpdate += npQueryService.batchUpdate("updateNPQueryService",
					inDs);
			resultUpdate += npQueryService.batchRemove("deleteNPQueryService",
					inDs);
	...
	private void insertDataSet() throws QueryException {
		...
		Assert.assertEquals("Fail to insert NPDataSet.", 3, resultInsert);
	...
	private void findListDataSet(int expected) throws QueryException {
		DataSet resultDataSet = npQueryService.search("findListNPQueryService",
				makeSelectDataSet("%bbnydory%"));
		Assert.assertEquals("Fail to find NPDataSet.", expected, resultDataSet
				.getRowCount());
	...
	private DataSet makeInsertDataSet() {
		...
		insertDataSet.set(0, "TEST_VARCHAR2", "Anyframe NPQueryService Test.");
		...
		insertDataSet.set(1, "TEST_VARCHAR2", "Anyframe NPQueryService Test.");
		...
		insertDataSet.set(2, "TEST_VARCHAR2", "Anyframe NPQueryService Test.");
	...
	private DataSet makeUpdateDataSet() {
		...
		updateDataSet.set(0, "TEST_VARCHAR2",
				"Anyframe NPQueryService Test. - UPDATE");
	...
	private DataSet makeAllDataSet() {
		...
		allDataSet.set(1, "TEST_VARCHAR2", "Anyframe NPQueryService Test.");
		...
		allDataSet.set(2, "TEST_VARCHAR2", "Anyframe NPQueryService Test.");

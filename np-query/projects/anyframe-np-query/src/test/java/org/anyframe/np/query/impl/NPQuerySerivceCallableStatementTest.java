...
/**
 * TestCase Name : NPQueryServiceCallableStatementTest <br>
 * <br>
 * [Description] : CallableStatement를 실행시키고 결과값을 검증한다.<br>
 * [Main Flow]
 * <ul>
 * <li>#-1 Positive Case : NPQueryService의 execute() 메소드를 통해 DBMS에 정의된 Function
 * FUNC_RETURN_NUM를 호출하는 쿼리문을 실행시키고 결과값을 검증한다.</li>
 * <li>#-2 Positive Case : NPQueryService의 execute() 메소드를 통해 DBMS에 정의된 Procedure
 * PROC_TOCHAR를 호출하는 쿼리문을 실행시키고 결과값을 검증한다.</li>
 * <li>#-3 Positive Case : NPQueryService의 execute() 메소드를 통해 DBMS에 정의된 Package
...
public class NPQuerySerivceCallableStatementTest {
	...
	public void testFunction() {
		...
		DataSetList outDl = npQueryService.execute("callFunction", inDs);
	...
	/**
	 * [Flow #-2] Positive Case : NPQueryService의 execute() 메소드를 통해 DBMS에 정의된
	...
	public void testProcedure() {
		...
		// 2. execute query
		DataSetList outDl = npQueryService.execute("callProcedure", inDs);
		DataSet outDs = outDl.get("outVal");
		...
		Assert.assertEquals("Anyframe NPQueryService Procedure Test", outDs
				.getString(0, "outVal"));
	}

	/**
	 * [Flow #-3] Positive Case : NPQueryService의 execute() 메소드를 통해 DBMS에 정의된
	...
	public void testPackage() {
		...
		// 2. execute query
		DataSetList outDl = npQueryService.execute("callPackage", inDs);
	...
	/**
	 * [Flow #-4] Positive Case : NPQueryService의 execute() 메소드를 통해 DBMS에 정의된
	...
	public void testPackageWithNullValue() {
		...
		DataSetList outDl = npQueryService.execute("callPackageWithNullValue");
		DataSet outDs1 = outDl.get("outVal");

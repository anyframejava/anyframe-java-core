...
import org.anyframe.np.query.util.ReflectionHelp;
import org.anyframe.np.query.util.NPDataBinder;
...
/**
 * The Util class needed at UI development using the Tobesoft's NPlatform
 * <p>
 * The NPlatform's data trasmitting object are DataSet and VariableList. So, it
 ...
 * VariableList and VO for NPMapper.
 * 
 * @author Byunghun Woo
 */
public class NPMapper {
	...
	/**
	 * <p>
	 * This method converts Value Object(VO) List into Dataset(NPlatform).
	 * <p>
	 * 
	 * @param dataSetName
	 *            a name of Dataset
	...
	/**
	 * <p>
	 * This method converts Value Object(VO) into DataSet(NPlatform).
	 * </p>
	 * 
	 * @param dataSetName
	 *            a name of DataSet
	...
	private static void populate(Object vo, VariableList variableList,
		...
		new NPDataBinder(vo, converToCamelCase).bind(variableList);
	}
	...
	private static Collection<Object> populate(Class<?> voClazz,
		...
		for (int i = 0; i < rowCount; i++) {
			...
			list.add(vo);
			new NPDataBinder(vo, converToCamelCase).bind(dataList, i);
		}
		return list;
	}
	...
	private static Map<String, List<Object>> populateCudList(Class<?> voClazz,
		...
		for (int i = 0; i < deleteRowCount; i++) {
			vo = voClazz.newInstance();
			deleteList.add(vo);
			new NPDataBinder(vo, converToCamelCase).bind(dataList, i, true);
		}
		return resultMap;
	}

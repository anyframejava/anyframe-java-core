...
/**
 * TestCase Name : XPServiceProcedureTest<br>
 * <br>
 * [Description] : BUG로 등록된 [AEJOLTP-170] 이슈에 대한 처리 결과를 검증한다.<br>
 * Procedure 호출을 위해서 XPQueryService.execute 메소드 호출 시, <br>
 * Procedure는 정상적으로 호출이 되지만 결과 DataSet을 가져올 때 NullPointer가 발생하는 버그<br>
 *
 * [원인] <br>
 * XPQueryService에서 OUT Parameter 명으로 DataSet을 생성하는데,
 * XPService에서는 querySet 이름으로 결과 DataSet을 가져오면서 NullPointerException 발생
 *
 * [수정 및 기능 추가] <br>
 ...
 *
 * 2. 기존 XPServiceImpl에서 잘못된 DataSet 이름으로 결과 DataSet을 가져오는 부분 수정 <br>
 *    querySet1_outParamName으로 결과 DataSet을 가져오도록 함 <br>
 *
 * 3. 기존 XPQueryServiceImpl의 execute 로직에서 중복된 로직을 리펙토링함  <br>
 *
 *
 * [Main Flow]
 * <ul>
 * <li>#-1 Positive Case : XPQueryService.execute() 메소드를 실행 시킨다.
...
public class XPServiceProcedureTest {
    ...
    public void testWithOutVariable() throws Exception {
        ...
        xpService.execute(inVl, inDl, outVl, outDl);

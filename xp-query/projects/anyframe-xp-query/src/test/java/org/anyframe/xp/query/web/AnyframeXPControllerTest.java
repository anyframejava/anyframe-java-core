...
/**
 * TestCase Name : AnyframeXPControllerTest <br>
 * <br>
 * [Description] : AnyframeXPController를 확장한 Controller 클래스의 메소드가 정상적으로 호출 되는지
 * 검증한다.<br>
 * [Main Flow]
 * <ul>
 * <li>#1- Positive Case : AnyframeXPController를 상속 받은 XPController의 process가 호출
 * 되었을 때 AnyframeXPController의 process method에서 HttpRequest로 넘어온 사용자 입력값을 입출력
 * DataSetList, VariableList를 분리하는 것을 검증한다.</li>
 * <li>#2- Positive Case : process method가 호출 되었을 때 AnyframeXPController를
 * extends한 XPController의 operate가 제대로 호출 되고 결과 메세지를 output VariableList에 저장하는
 * 것을 검증한다.</li>
 * <li>#3- Positive Case : XPController의 operate method에서 Exception이 발생했을 경우
 * output VariableList에 Exception 메세지가 저장되는 것을 검증한다.</li>
 * <li>#4- Positive Case : Sever Side Value전달 객체인 VO클래스를 XPlatform의 Dataset로
 * 변환하는 함수 convertVoToDataset을 검증한다. 값이 미리 세팅되어 있는 XPTestVO를 'test_ds'란 아이디의
 * Dataset으로 변환 한 후 Dataset값과 XPTestVO의 값들을 비교하여 타입에 따라서 값이 제대로 매핑 되는지 확인한다.</li>
...
public class AnyframeXPControllerTest extends TestCase {
	...
	/**
	 * AnyframeXPController를 상속받은 XPController를 맴버 변수로 선언한다.
	 */
	private TestXPController controller;

	/**
	 * 테스트를 위해 XPController의 인스턴스를 생성한다.
	 */
	protected void setUp() throws Exception {
		controller = new TestXPController();
	}

	/**
	 * [Flow #-1] Positive Case : AnyframeXPController를 상속 받은 XPController의
	 * process가 호출 되었을 때 AnyframeXPController의 process method에서 HttpRequest로 넘어온
	...
	/**
	 * [Flow #-2] Positive Case : process method가 호출 되었을 때 AnyframeXPController를
	 * extends한 XPController의 operate가 제대로 호출 되고 결과 메세지를 output VariableList에
	 * 저장하는 것을 검증한다.
	 * 
	 * @throws Exception
	 */
	...
	/**
	 * [Flow #-3] Positive Case : XPController의 operate method에서 Exception이 발생했을
...
//	}
	...
	/**
	 * [Flow #-4] Positive Case : Sever Side Value전달 객체인 VO클래스를 XPlatform의
	 * Dataset로 변환하는 함수 convertVoToDataset을 검증한다. 값이 미리 세팅되어 있는 XPTestVO를
	 * 'test_ds'란 아이디의 Dataset으로 변환 한 후 Dataset값과 XPTestVO의 값들을 비교하여 타입에 따라서 값이
	 * 제대로 매핑 되는지 확인한다.
	 * 
	 * @throws Exception
	 */
	public void testConvertVoToDataset() throws Exception {
		...
		DataSet ds = XPMapper.convertVoToDataset("test_ds", initTestVO(), true);
	...
	public void testConvertVoListToDataset() throws Exception {
		...
		List<XPTestVO> voList = new ArrayList<XPTestVO>();

		voList.add(initTestVO());
		voList.add(initTestVO());
		voList.add(initTestVO());

		DataSet ds = XPMapper.convertVoListToDataset("test_ds", voList, true);
	...
	public void testConvertDatasetToListMap() throws Exception {
		...
		Map<String, List<Object>> map = XPMapper.convertDatasetToListMap(
				MappingVO.class, ds);
	 ...
	 */
	class TestXPController extends AbstractXPController {
		...
		private Logger log = LoggerFactory.getLogger(TestXPController.class);

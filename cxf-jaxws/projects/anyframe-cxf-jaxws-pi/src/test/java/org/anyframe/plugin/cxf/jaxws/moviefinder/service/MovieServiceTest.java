 ...
 */
package org.anyframe.plugin.cxf.jaxws.moviefinder.service;
...
import org.anyframe.plugin.cxf.jaxws.domain.Genre;
...
public class MovieServiceTest {
	...
	public void manageMovie() throws Exception {
		...
		// 2. assert - create
		Map<String, Movie> movieMap = movieService.get(movie.getMovieId());
		assertNotNull("fail to fetch a movie", movieMap);
		assertEquals("fail to compare a movie title", "Shrek (2001)",
				movieMap.get("movie").getTitle());
		...
		// 4. assert - update
		movieMap = movieService.get(movie.getMovieId());
		assertNotNull("fail to fetch a movie", movieMap);
		assertEquals("fail to compare a updated title", title, movieMap.get("movie").getTitle());
	...
	public void findMovie() throws Exception {
		Map<String, Movie> movieMap = movieService.get("MV-00001");
		assertNotNull("fail to fetch a movie", movieMap);
	}

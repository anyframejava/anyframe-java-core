...
<%@ include file="/sample/common/top.jsp"%>
		<div class="location"><a href="<c:url value='/anyframe.jsp'/>">Home</a> &gt; <a href="<c:url value='/cxfJaxWsMovieFinder.do?method=list'/>">CXF-JAXWS 1.0.6-SNAPSHOT</a></div>
    </div>
    <hr />
<script type="text/javascript" src="<c:url value='/sample/javascript/CommonScript.js'/>"></script>    
<script type="text/javascript">
		function fncCreateMovieView() {
			document.location.href="<c:url value='/cxfJaxWsMovie.do?method=createView'/>";
		}
		function fncSearchMovie() {
		   	document.searchForm.action="<c:url value='/cxfJaxWsMovieFinder.do?method=list'/>";
                ...
                <tbody>
                	<c:forEach var="movie" items="${movies}">
	                	<tr>
	                        ...
	                        <td><a class="linkClass" href="${ctx}/cxfJaxWsMovie.do?method=get&amp;movieId=${movie.movieId}">${movie.title}</a></td>
	                        <td>${movie.director}</td>
	                        <td>${movie.actors}</td>
	                        <td class="align_center">${movie.ticketPrice}</td>
	                        <td class="align_center"><fmt:formatDate pattern="yyyy-MM-dd" value="${movie.releaseDate}" /></td>
	                    </tr>
                    </c:forEach>
                </tbody>
            </table>

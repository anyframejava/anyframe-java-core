...
<%@ include file="/sample/common/top.jsp"%>
		<div class="location"><a href="<c:url value='/anyframe.jsp'/>">Home</a> &gt; <a href="<c:url value='/logmanagerMovieFinder.do?method=list'/>">Log Manager 1.0.0</a></div>
    </div>
    <hr />
...
<script type="text/javascript">
	function fncCreateMovie() {
	    document.movieForm.action="<c:url value='/logmanagerMovie.do?method=create'/>";
	    document.movieForm.submit();
	}
	
	function fncUpdateMovie() {
	    document.movieForm.action="<c:url value='/logmanagerMovie.do?method=update'/>";
	    document.movieForm.submit();
	}
	
	function fncRemoveMovie(){	
		if(confirmDelete('movie')) {
		    document.movieForm.action="<c:url value='/logmanagerMovie.do?method=remove'/>";
		    document.movieForm.submit();
		}	    
	}	
	function fncLogOut(){
	    ...
	    ans = confirm(msg);
	    if (ans) {
	    	document.location.href="<c:url value='/logmanagerLogout.do'/>";
	    } else {
	        return false;
	    }
	}
	 ...
	 <div class="listunder_container">           
        <div class="list_underbtn_right">
            <span class="button default icon">
                ...
                <a href="javascript:fncLogOut();">Logout</a>
            </span>
        </div>
    </div>
    <br/><br/>
    <div id="container">
        ...
        <span class="button default icon">
            ...
            <a href="<c:url value='/logmanagerMovieFinder.do?method=list'/>"><spring:message code="movie.button.list" /></a>
        </span>
        <c:if test="${empty movie.movieId}">
	        <span class="button default icon">
	            <span class="add"></span>
	            <a href="javascript:fncCreateMovie();"><spring:message code="movie.button.add" /></a>
	        </span>
        </c:if>
        <c:if test="${not empty movie.movieId}">
        	<span class="button default icon"> 
	            <span class="update"></span>
	            <a href="javascript:fncUpdateMovie();"><spring:message code="movie.button.update" /></a>
	        </span>
	         <span class="button default icon">
	            <span class="delete"></span>
	            <a href="javascript:fncRemoveMovie();"><spring:message code="movie.button.remove" /></a>
	        </span>
        </c:if>
    	</div>
	</div>

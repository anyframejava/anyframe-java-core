...
<%@ include file="/sample/common/top.jsp"%>
		<div class="location"><a href="<c:url value='/anyframe.jsp'/>">Home</a> &gt; <a href="<c:url value='/genericMovieFinder.do?method=list'/>">Generic 1.5.2-SNAPSHOT</a></div>
    </div>
    <hr />
...
<script type="text/javascript">
	function fncCreateMovie() {
	    document.genericMovieForm.action="<c:url value='/genericMovie.do?method=create'/>";
	    document.genericMovieForm.submit();
	}
	
	function fncUpdateMovie() {
	    document.genericMovieForm.action="<c:url value='/genericMovie.do?method=update'/>";
	    document.genericMovieForm.submit();
	}
	
	function fncRemoveMovie(){	
		if(confirmDelete('genericMovie')) {
		    document.genericMovieForm.action="<c:url value='/genericMovie.do?method=remove'/>";
		    document.genericMovieForm.submit();
		}	    
	}	
</script>    
    <div id="container">
    	<div class="cont_top">
        	<h2>
        		<c:if test="${empty genericMovie.movieId}">
				 	<spring:message code='movie.add'/>
				 	<c:set var="readonly" value="false"/>
				</c:if>
			
				<c:if test="${not empty genericMovie.movieId}">	
        ...
        <div class="view">
        <form:form modelAttribute="genericMovie" name="genericMovieForm" method="post">
        <c:if test="${not empty genericMovie.movieId}">
                    ...
                    <tr>
                        ...
                        <td>
                        	<form:select id="genre" path="genre.genreId">
            					<form:options items="${genericGenreList}" itemValue="genreId" itemLabel="name"/>
				          	</form:select>
          				</td>
                    </tr>
                    <tr>
        ...
        <div class="btncontainer_center">
		    <a href="<c:url value='/genericMovieFinder.do?method=list'/>">

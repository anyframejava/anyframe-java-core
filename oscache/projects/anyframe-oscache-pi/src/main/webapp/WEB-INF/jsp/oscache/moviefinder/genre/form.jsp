...
<%@ include file="/sample/common/top.jsp"%>
		<div class="location"><a href="<c:url value='/anyframe.jsp'/>">Home</a> &gt; <a href="<c:url value='/oscacheGenre.do?method=list'/>">OSCache 1.5.2-SNAPSHOT</a></div>
    </div>
    <hr />
...
<script type="text/javascript">
	function fncCreateGenre() {
	    document.genreForm.action="<c:url value='/oscacheGenre.do?method=create'/>";
	    document.genreForm.submit();
	}
	
	function fncUpdateGenre() {
	    document.genreForm.action="<c:url value='/oscacheGenre.do?method=update'/>";
	    document.genreForm.submit();
	}
	
	function fncRemoveGenre(){	
		if(confirmDelete('genre')) {
		    document.genreForm.action="<c:url value='/oscacheGenre.do?method=remove'/>";
		    document.genreForm.submit();
		}	    
	}	
	</script>    
 ...
 <div class="view">
	...
	<table summary="This table shows detail information about genre, title, director, actors, runtime, release date, ticket price of the movie">
	    ...
	    <a href="<c:url value='/oscacheGenre.do?method=list'/>">

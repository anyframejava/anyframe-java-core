...
public class AjaxParamTilesView extends AjaxTilesView {
	...
	protected void flattenAttributeMap(BasicTilesContainer container,
			TilesRequestContext requestContext, Map resultMap,
			Definition compositeDefinition, HttpServletRequest request,
			HttpServletResponse response) throws Exception {

		if (compositeDefinition != null
				&& request.getAttribute("isNestedDefinition") == null) {
			if (compositeDefinition.getAttributes() != null
					&& compositeDefinition.getAttributes().size() > 0) {
				Iterator i = compositeDefinition.getAttributes().keySet()
						.iterator();
				while (i.hasNext()) {
					Object key = i.next();
					Attribute attr = (Attribute) compositeDefinition
							.getAttributes().get(key);
					AttributeType attrType = attr.getType() != null ? attr
							.getType() : detectType(container, requestContext,
							attr);
					if (AttributeType.DEFINITION.equals(attrType)
							|| AttributeType.TEMPLATE.equals(attrType)
							|| AttributeType.OBJECT.equals(attrType)) {
						resultMap.put(key, attr);
						if (AttributeType.DEFINITION.equals(attrType)) {
							Definition nestedDefinition = container
									.getDefinitionsFactory().getDefinition(
											attr.getValue().toString(),
											requestContext);
							if (nestedDefinition != null
									&& nestedDefinition != compositeDefinition) {
								flattenAttributeMap(container, requestContext,
										resultMap, nestedDefinition, request,
										response);
							}
						}
					}
			...
			AttributeContext attributeContext = container
					.getAttributeContext(new Object[] { request, response });
			...
			Definition originalDefinition = container.getDefinitionsFactory()
					.getDefinition(url, requestContext);

			Set<String> attrKeySet = originalDefinition.getAttributes()
					.keySet();

			Iterator<String> attrKeyItr = attrKeySet.iterator();

			Map<String, String> tilesAttrMap = (Map<String, String>) request
					.getAttribute("tilesAttrMap");
			while (attrKeyItr.hasNext()) {
				String attrKey = (String) attrKeyItr.next();
				Attribute attribute = new Attribute();

				if (tilesAttrMap != null && tilesAttrMap.containsKey(attrKey))
					if (isValidDefinition(requestContext, tilesAttrMap
							.get(attrKey), container)) {
					...
					attribute.setValue(originalDefinition.getAttribute(attrKey)
							.getValue());
				...
				if (AttributeType.DEFINITION.equals(attribute.getType())) {
					...
					if (nestedDefinition != null
						...
						flattenAttributeMap(container, requestContext,
								resultMap, nestedDefinition, request, response);
					}
				}
			}
			// Process dynamic attributes
	...
	private void setValueWithPrefixSuffix(Attribute attribute, Object value,
		...
		logger.debug("View name is '" + prefix + value + suffix
	...
	protected void renderMergedOutputModel(Map model,
		...
		List<String> renderValuesList = renderValuesArray != null ? Arrays
				.asList(renderValuesArray) : null;
		...
		if (renderValuesList != null && renderValuesList.contains("all"))
			...
			if (attrNames.length == 0) {
				logger
						.warn("An Ajax request was detected, but no fragments were specified to be re-rendered.  "
			...
			ELTilesContainer container = (ELTilesContainer) TilesAccess
			...
			TilesRequestContext tilesRequestContext = container
					.getTilesContextFactory().createRequestContext(
			...
			flattenAttributeMap(container, tilesRequestContext,
					flattenedAttributeMap, compositeDefinition, request,
					response);
			...
			for (int i = 0; i < attrNames.length; i++) {
				Attribute attributeToRender = (Attribute) flattenedAttributeMap
						.get(attrNames[i]);

				if (attributeToRender == null) {
					throw new ServletException(
							"No tiles attribute with a name of '"
									+ attrNames[i]
									+ "' could be found for the current view: "
									+ this);
				} else {
					container.render(attributeToRender, response.getWriter(),
							new Object[] { request, response });
				}
			}
		} else
	...
	private void renderTilesPage(Map model, HttpServletRequest request,
			HttpServletResponse response, ServletContext servletContext)
			throws ServletException, Exception, TilesException {
	...
	private boolean isValidDefinition(TilesRequestContext context,
			String definitionName, BasicTilesContainer container) {
		try {
			Definition definition = container.getDefinitionsFactory()
					.getDefinition(definitionName, context);
	...
	private AttributeType detectType(BasicTilesContainer container,
			TilesRequestContext requestContext, Attribute attr)
			throws DefinitionsFactoryException {
		if (attr.getValue() instanceof String) {
			if (container.getDefinitionsFactory().getDefinition(
					attr.getValue().toString(), requestContext) != null) {

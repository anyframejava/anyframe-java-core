...
public class SimpleServiceController extends AbstractServiceController {
	...
	private static final Log LOG = LogFactory
			.getLog(SimpleServiceController.class);
	...
	protected String bindRequestsAndInvokeServiceMethod(ServiceEL serviceEL,
		...
		if (serviceEL.getArgList().size() > 0) {
			for (int i = 0; i < serviceEL.getArgList().size(); i++) {
					...
					if (StringUtil.isNotEmpty(request.getParameter("fileDir"))
							&& StringUtil.isNotEmpty(request
									.getParameter("hiddenUploadedFiles"))) {
						String path = this.getUploadPath()
								+ request.getParameter("fileDir") + "/";

						// make filename string array
						String hiddenUploadedFiles = request
								.getParameter("hiddenUploadedFiles");
						String[] fileNames = StringUtil.getStringArray(
								hiddenUploadedFiles, ",");
						String filePaths = "";
						for (int j = 0; j < fileNames.length; j++) {
							// String encodeFileName = URLEncoder.encode(fileNames[i],
							// "utf-8");
						...
						String filePathsAttrName = request
								.getParameter("filePathsAttrName");
						if (StringUtil.isEmpty(filePathsAttrName))
							filePathsAttrName = "FilePaths";
						else
							filePathsAttrName = filePathsAttrName.substring(0, 1)
									.toUpperCase()
									+ filePathsAttrName.substring(1);

						// call reflection api to call setter method for filepath
						Method method = ReflectionUtils.findMethod(command[i].getClass(),
								"set" + filePathsAttrName, String.class);
						if (method != null)
	...
	protected Object[] bindingCommand(HttpServletRequest request,
		...
		Object[] command = new Object[serviceEL.getArgList().size()+1];
					...
					if (StringUtil.isNotEmpty(paramName) && argClass != null) {
						if (argClass.isArray()) {
							command[i] = request.getParameterValues(paramName
								...
								|| argClass.getPackage().getName().equals(
										"java.lang")) {
			...
			LOG.error("fail to bind command.", e);
		}
		return command;
	}

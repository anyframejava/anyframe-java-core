 ...
 * 
 */
public abstract class AbstractServiceController extends MultiActionController
		implements MessageSourceAware {

	/** Logger that is available to AbstractServiceController */
	private final Log logger = LogFactory
			.getLog(AbstractServiceController.class);
	...
	protected void checkDuplicatedSubmission(HttpServletRequest request)
		...
		if (isShowNewForm != null && isShowNewForm.equals("true")) {
			if (formName == null)
				...
				if (getLogger().isDebugEnabled()) {
					getLogger().debug(
							"Setting form session attribute [" + formName
									+ "] ");
				}
			}
		}
	...
	protected void setCompleteDuplicatedSubmission(HttpServletRequest request)
			throws BaseException {
		HttpSession currentSession = request.getSession();
		...
		if (isSessionForm != null && isSessionForm.equals("true")) {
			if (formName == null)
				...
				if (currentSession.getAttribute(formName) != null) {
					...
					throw new BaseException(new String(getMessageSource()
							.getMessage("common.msg.invalidtoken.error",
	...
	protected Map<String, Object> executeServiceAndReturnView(
		...
		if (StringUtil.isNotEmpty(tiles)) {
			...
			while (inputPageListItr.hasNext()) {
				List<String> value = StringUtil.getTokens(inputPageListItr
						.next().toString(), ":");
		...
		if (resultView == null) {
			if (StringUtil.isNotEmpty(viewName)
					&& (viewName.startsWith("forward:/") || viewName
							.startsWith("redirect:/"))) {
				resultView = viewName;
				...
				resultView = view;
			else if (StringUtil.isEmpty(view)
					&& StringUtil.isNotEmpty(viewName))
				resultView = viewName;
		}
		...
		if (StringUtil.isEmpty(serviceParam)) {
			...
			if (request.getAttribute(PARAM_ATTR_SERVICE) != null) {
				serviceParam = (String) request
						.getAttribute(PARAM_ATTR_SERVICE);
			} else
				return viewInfo;
		}
		...
		BeanMethodInfo methodInfo = getBeanMethodsRepo().findBeanMethod(
				serviceEL.getBeanName(), serviceMethod);

		Object[] command = bindingCommand(request, methodInfo, serviceEL);

		if (StringUtils.isEmpty(serviceFromView)
				&& request.getParameter(PARAM_VALIDATOR) != null) {
			if (request.getParameter(PARAM_INPUT_PAGE) == null
					|| request.getParameter(PARAM_INPUT_PAGE).length() == 0) {
				if (logger.isDebugEnabled()) {
					logger
							.debug("'inputPage' is required or use javascript function.");
				}
			} else {
				boolean isValidate = excuteValidator(request, command[0],
						new BeanPropertyBindingResult(command[0], methodInfo
								.getInputParamName()));
				if (isValidate == false) {

					if (request.getAttribute(ATTR_TILES_MAP) != null) {
						String tilesLayout = request
								.getParameter(PARAM_ATTR_LAYOUT);
						if (request.getParameter(PARAM_ATTR_LAYOUT) != null) {
							tilesLayout = request
									.getParameter(PARAM_ATTR_LAYOUT);
						} else if (request.getAttribute(PARAM_ATTR_LAYOUT) != null) {
							tilesLayout = (String) request
									.getAttribute(PARAM_ATTR_LAYOUT);
						}
						viewInfo.put("VIEW_NAME", tilesLayout);
					} else
						viewInfo.put("VIEW_NAME", request
								.getParameter(PARAM_INPUT_PAGE));
	...
	protected abstract Object[] bindingCommand(HttpServletRequest request,
			BeanMethodInfo methodInfo, ServiceEL serviceEL) throws Exception;

	protected String executeServiceMethod(ServiceEL serviceEL,
			HttpServletRequest request, String resultName, Object[] command,
			BeanMethodInfo methodInfo) throws Exception {
		// check method info
		if (methodInfo == null) {
			if (getLogger().isDebugEnabled()) {
				getLogger().debug("Invalid Service Method is called.");
			}
			throw new BaseException(new String(getMessageSource().getMessage(
					"common.msg.invalidmethod.error", new String[] {},
					Locale.getDefault())));
		}
		...
		if (methodInfo.getInputParam() != null
				&& methodInfo.getInputParam()[0] != null)
			return bindRequestsAndInvokeServiceMethod(serviceEL, request,
					argument, methodInfo, resultName, command);

		return invokeServiceMethod(serviceEL, request, new Object[1], null, methodInfo,
				resultName);
	}

	protected abstract String bindRequestsAndInvokeServiceMethod(
			ServiceEL serviceEL, HttpServletRequest request, Object[] argument,
	...
	private boolean excuteValidator(HttpServletRequest request, Object command,
		...
		request.setAttribute(bindingResult.getObjectName(), bindingResult
				.getTarget());

		if (StringUtil.isNotEmpty(validatorBeanId)) {
			Validator springmvcValidator = (Validator) this
					.getApplicationContext().getBean(validatorBeanId);
	...
	public void validate(Object target, Errors errors) {
		Set<ConstraintViolation<Object>> result = this.validator
				.validate(target);
		for (ConstraintViolation<Object> violation : result) {
			...
			if (fieldError == null || !fieldError.isBindingFailure()) {
				try {
					Annotation annotation = (Annotation)violation
					.getConstraintDescriptor().getAnnotation();
					errors
							.rejectValue(field,annotation
									.annotationType().getSimpleName(),
									getArgumentsForConstraint(errors
											.getObjectName(), field, violation
	...
	protected void setTilesAttributes(HttpServletRequest request,
			String inputpage) {
		String tiles = "";
		if (inputpage != null)
			tiles = inputpage;
	...
	protected void setInitdataAttributes(HttpServletRequest request)
			throws Exception {
		if (StringUtil.isNotEmpty(request.getParameter(PARAM_INITDATA_SERVICE))) {
			String initdataService = request
					.getParameter(PARAM_INITDATA_SERVICE);
			...
			for (int i = 0; i < services.size(); i++) {
				...
				BeanMethodInfo methodInfo = getBeanMethodsRepo()
						.findBeanMethod(serviceEL.getBeanName(), serviceMethod);

				Object[] command = bindingCommand(request, methodInfo, serviceEL);

				executeServiceMethod(serviceEL, request, results.get(i),
						command, methodInfo);
			}
		}
	}

	protected void setCommandAttributes(HttpServletRequest request)
			throws ClassNotFoundException {
		if (StringUtil.isNotEmpty(request.getParameter(PARAM_COMMAND_NAME))) {
			Class<?> argClass = Class.forName(request
					.getParameter(PARAM_COMMAND_CLASS));
			Object command = BeanUtils.instantiateClass(argClass);
			request.setAttribute(request.getParameter(PARAM_COMMAND_NAME),
					command);
		}
	}
	...
	protected int getPageIndex(HttpServletRequest request) {
		if (StringUtil.isNotEmpty(request.getParameter(PARAM_PAGE_INDEX)))
			return new Integer(request.getParameter(PARAM_PAGE_INDEX))
					.intValue();

...
public class PageTilesContainer extends BasicTilesContainer {
	...
	private static final Log LOG = LogFactory.getLog(PageTilesContainer.class);
	...
	public void render(String definitionName, Object... requestItems)
		...
		TilesRequestContext requestContext = getContextFactory()
	...
	protected void render(TilesRequestContext request, String definitionName)
		...
		if (!isValidDefinition(request, definitionName)) {
			...
			if (map.get("layout") == null) {
				definitionName = (String) ((HttpServletRequest) request
						.getRequest()).getAttribute("layout");
			} else {
				definitionName = (String) map.get("layout");
			}

			Definition originalDefinition = getDefinition(definitionName,
					request);
			...
			Map<String, String> tilesAttrMap = (Map<String, String>) ((HttpServletRequest) request
					.getRequest()).getAttribute("tilesAttrMap");
			while (attrKeyItr.hasNext()) {
				...
				if (tilesAttrMap != null && tilesAttrMap.containsKey(attrKey)) {
					if (isValidDefinition(request, tilesAttrMap.get(attrKey))) {
						...
						setValueWithPrefixSuffix(attribute, tilesAttrMap
								.get(attrKey));
					}
				} else
	...
	private void setValueWithPrefixSuffix(Attribute attribute, Object value) {
		...
		attribute.setValue(this.initParams.get("org.apache.tiles.prefix")
				+ value + this.initParams.get("org.apache.tiles.suffix"));
	}

	private void prepare(TilesRequestContext context, String preparerName,
			boolean ignoreMissing) throws TilesException {

		if (LOG.isDebugEnabled()) {
			LOG.debug("Prepare request received for '" + preparerName);
		}

		ViewPreparer preparer = this.getPreparerFactory().getPreparer(
				preparerName, context);

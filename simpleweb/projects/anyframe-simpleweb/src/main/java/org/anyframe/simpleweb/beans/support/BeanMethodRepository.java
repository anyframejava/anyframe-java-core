 ...
 * 
 */
public class BeanMethodRepository implements ApplicationContextAware,
		InitializingBean {
	private static final Log LOG = LogFactory
			.getLog(BeanMethodRepository.class);
	...
	public void createInterfaceMathodMap(String beanName, Object bean,
		...
		for (int i = 0; i < methods.length; i++) {
			...
			if (beansMethodMap.get(beanName + "." + methods[i].getName()) == null) {
				beansMethodMap.put(beanName + "." + methods[i].getName(),
						beanMethodInfo);
				if (LOG.isDebugEnabled())
					LOG.debug("Resolve method name ='" + beanName + "."
							+ methods[i].getName() + "'");
			}
		}
	...
	private Class<?>[] findParamClass(Type enclosingType, Type type,
			Class<?> interfaceClass, Method method, String paramType) {
		Class<?>[] paramClass = new Class<?>[2];

		try {
			paramClass[0] = ((Class<?>) TypeUtil.getRawType(enclosingType,
					type));
			if (type instanceof ParameterizedType) {
				Type args = ((ParameterizedType) type).getActualTypeArguments()[0];

				paramClass[1] = (Class<?>) TypeUtil.getRawType(enclosingType,
						args);
			}
		} catch (Exception e) {
			LogFactory.getLog(this.getClass()).warn(
					"Fail to find "+ paramType + " parameter class type from method = ["
							+ interfaceClass.getCanonicalName() + "."
							+ method.getName() + "], parameter type = ["+type + "]");
		}
		return paramClass;
	}	
	...
	public void afterPropertiesSet() throws Exception {
		this.beansMethodMap = new ConcurrentCache<String, BeanMethodInfo>(
				this.cacheSize);
	}

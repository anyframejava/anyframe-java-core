		...
		.declare(
				...
				{	
					...
					submit : function(_12) {
						...
						if (this.formId == "") {
							Spring.remoting.getLinkedResource(this.sourceId, this.params, this.popup, this.func);
						} else {
							if (this.allowed) {
								Spring.remoting.submitForm(this.sourceId, this.formId, this.params, this.func);
							}
						}
						dojo.stopEvent(_12);
					},
...
dojo.declare("Spring.RemotingHandler", Spring.AbstractRemotingHandler, {
	...
	submitForm : function(_14, _15, _16, func) {
		
		//alert("submitForm----");
		  
		// default values
		var paramFragments = "body";
		...
		for ( var key in _16) {
			_17[key] = _16[key];
			if(key == "fragments") paramFragments = _17[key];
			else if(key == "popupTitle") popupTitle = _17[key];
			else if(key == "popupWindow") popupWindow = _17[key];
			else if(key == "popupClose") popupClose = _17[key];
		}
		...
		//alert("after func: proceed->"+proceed);
	...
	getLinkedResource : function(_1c, _1d, _1e, func) {
		this.getResource(dojo.byId(_1c).href, _1d, _1e, func);
	},
	getResource : function(_1f, _20, _21, func) {
		// call javascript function for 
		var proceed = true;
		if(func != "")
			proceed = func.call();
		if(proceed != false){
	...
	handleResponse : function(_22, _23) {
		...
		var popupClose = true;
		var popupTitle="";
		if(_23.args.content.popupWindow != undefined) popupWindow = _23.args.content.popupWindow;
		if(_23.args.content.popupClose != undefined) popupClose = _23.args.content.popupClose;
		if(_23.args.content.popupTitle != undefined) popupTitle = _23.args.content.popupTitle;
		
		//alert("popupWindow->"+popupWindow+", popupClose->"+popupClose);
		
		
		if(document.getElementById(popupWindow) != undefined && document.getElementById(popupWindow).parentNode.parentNode.id != undefined) {				
			//alert("document.getElementById(popupWindow)->"+document.getElementById(popupWindow));
			//alert("document.getElementById(popupWindow).parentNode->"+document.getElementById(popupWindow).parentNode);
			//alert("document.getElementById(popupWindow).parentNode.parentNode->"+document.getElementById(popupWindow).parentNode.parentNode);
			//alert("document.getElementById(popupWindow).parentNode.parentNode.id->"+document.getElementById(popupWindow).parentNode.parentNode.id);
			
			if(popupClose)
				dijit.byId(document.getElementById(popupWindow).parentNode.parentNode.id).destroyRecursive(false);
		}  
		...
		_22 = _22.replace(_2b, "");
		if (_26) {
			Spring.remoting._renderResponseToModalDialog(_22, popupTitle);
		} else {
	...
	_renderResponseToModalDialog : function(_3a, popupTitle) {
		dojo.require("dijit.Dialog");
		var _3b = new dijit.Dialog( {title:popupTitle});
		_3b.setContent(_3a);
...
function partialRenderListItems(css, dynamicAttrName) {
  dojo.query("."+css).forEach(function(element) {;
	Spring.addDecoration(new Spring.AjaxEventDecoration({
	elementId: element.id, 
	event: "onclick", 
	params: {fragments: dynamicAttrName} 
	})); 
  }); 
}	